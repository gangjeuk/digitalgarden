---
{"dg-publish":true,"permalink":"/jp/CTF/Basic/ELF ファイル形式からRelocation Linkingまで/","tags":["CTF/basic","TODO/got_plt_tls_파일내용과_같이_검토_필요"],"created":"2023-10-10","updated":"2023-10-28"}
---
...
여기서 자세히 보아야 하는 값은 앞으로 나올 [[jp/CTF/Basic/ELF ファイル形式からRelocation Linkingまで#Section header entry\|Section]]과 [[jp/CTF/Basic/ELF ファイル形式からRelocation Linkingまで#Program\|Program]]에서 다루어질 Section header, Program header의 위치를 가리키는 *e_phoff*, *e_shoff*이다.
...
먼저 앞에서 설명한 [[jp/CTF/Basic/ELF ファイル形式からRelocation Linkingまで#Section header entry\|Section header entry]]를 다시 한번 상기하자.
...
실제 재배치에 필요한 정보를 가지는  Symbol Table Section 과 Relocation Section은 [[jp/CTF/Basic/ELF ファイル形式からRelocation Linkingまで#File header\|위와]]같이 각각 엔트리로 구성되어 있다. 
...
## Address Calculation
실제 주소의 계산은 [[jp/CTF/Basic/ELF ファイル形式からRelocation Linkingまで#Relocation Entry(.rela, .rel)\|Relocation Entry]]장에서 설명한  r_info를 이용해서 이루어진다.
...
## Procedure
사용 프로그램으로는 [[jp/CTF/Basic/ELF ファイル形式からRelocation Linkingまで#실습 준비\|2 번째 프로그램(Global)]]을 사용할 것이다.
...
위에서 [[jp/CTF/Basic/ELF ファイル形式からRelocation Linkingまで#Address Calculation\|주어진 식]]을 조금 더 풀어 쓰면 다음과 같다.
$$
...
구조의 경우 [[jp/CTF/Basic/ELF ファイル形式からRelocation Linkingまで#Symbol Table Entry(.symtab)\|Symbol Table Entry]]와 동일한 구조를 가지고 구조체 또한 같은 구조체를 가진다.
